...
auto BufferPoolManager::NewPage() -> page_id_t {
  ...
  disk_manager_->WritePage(next_page_id_, data);
  auto ret = next_page_id_.load();
  next_page_id_++;
  return ret;
}
...
auto BufferPoolManager::DeletePage(page_id_t page_id) -> bool {
  std::scoped_lock latch(*bpm_latch_);
  auto fh = FindFrameHeaderFromPage(page_id);
  if (fh.has_value()) {
    auto fh_ptr = fh.value();
    if (fh_ptr->pin_count_ > 0) {
      return false;
    }
    FlushPageUnsafe(page_id);

    fh_ptr->data_.clear();
    fh_ptr->data_.shrink_to_fit();
    fh_ptr->page_id_.reset();
  
    replacer_->SetEvictable(fh_ptr->frame_id_,true);
    replacer_->Remove(fh_ptr->frame_id_);

    free_frames_.push_back(fh_ptr->frame_id_);

    page_table_.erase(fh_ptr->frame_id_);
  }
...
auto BufferPoolManager::FindAvailableFrame(page_id_t page_id, int type) -> std::optional<std::shared_ptr<FrameHeader>> {
  ...
  if (page_table_.count(page_id) != 0) {
    ...
    if (!free_frames_.empty()) {
      ...
      //std::cout << "evicted: " << fid.value() << std::endl;
    }
    fh = FindFrameHeaderFromFrame(fid.value());
  }

  if (!fid.has_value() || !fh.has_value()) {
    return std::nullopt;
  }
...
auto BufferPoolManager::CheckedReadPage(page_id_t page_id, AccessType access_type) -> std::optional<ReadPageGuard> {
  ...
  auto bpm_latch = bpm_latch_;
  auto disk_scheduler = disk_scheduler_;
  auto page_guard = ReadPageGuard(page_id, fh_ptr, replacer, bpm_latch, disk_scheduler);
...
auto BufferPoolManager::GetPinCount(page_id_t page_id) -> std::optional<size_t> {
  std::scoped_lock lk(*bpm_latch_);
  auto fh = FindFrameHeaderFromPage(page_id);
  if (fh.has_value()) return fh.value()->pin_count_;
  return std::nullopt;
}

auto BufferPoolManager::FindFrameHeaderFromPage(page_id_t page_id) -> std::optional<std::shared_ptr<FrameHeader>> {
  if (page_table_.count(page_id) == 0) {
    return std::nullopt;
  }
  frame_id_t frame_id = page_table_[page_id];
  for (auto& frame_header : frames_) {
...
auto BufferPoolManager::FindFrameHeaderFromFrame(frame_id_t frame_id) -> std::optional<std::shared_ptr<FrameHeader>> {
  for (auto& frame_header : frames_) {
    if (frame_header->frame_id_ == frame_id) {
      return frame_header;
    }
